...
採点は次の手順で、2軸を独立に行うこと：
1. まず sales（💼 営業プロ視点）＝本質的な質を採点する：模範に関係なく、お客様目線で
   見たトークそのものの完成度。
2. 次に reference（🎯 模範トーク視点）＝再現度を、上とは切り離して別基準で採点する：
   {reference_axis_note}
※この2軸は「絶対的な質」と「模範への似ている度」という別物のため、同じ商談でも
  点数がズレるのが普通。全5項目が両軸とも同点になるのは不自然なので、各項目で
  観点の違いを必ず点数にも反映する（コメントだけでなく数字も変える）。例：
...
- feedback の before / after は **営業担当の発言だけ** を書く。お客様の発言を営業の発言として書かない。
  （お客様の言葉・様子は emotion_note や hidden_needs に書く）
...
{{
  ...
  "feedback": [{{
    ...
    "emotion_note": "<その瞬間のお客様の感情・発言・様子はここに書く（きっかけとなったお客様の言葉もここ）>",
    "before": "<その場面で【営業担当（提案・ヒアリングをする側）】が実際に話した発言だけを、話した言葉のまま。お客様の発言は入れない。話者名の接頭辞（『◯◯:』等）は付けない>",
