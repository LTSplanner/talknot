...
#              「こう言えたらいいんだ」と一目で分かることを狙う。
# アプリ側（app.py）が INTRO_TURNS の STEP2 に載せた variants から1つをランダム選出し、
# セッション内で固定して お客様セリフ・音声・AI評価すべてに一貫して使う。
# 【将来拡張の余地】次段階では「マイクを押すと お客様→営業→お客様→営業 と本当に往復する」
# ロープレを想定。今回の会話見本はその下地であり、variant に往復の複数お客様セリフを
# 後から持たせられるよう、customer（第一声）と hint（見本会話）の役割を分けてある。
SMALLTALK_VARIANTS = [
